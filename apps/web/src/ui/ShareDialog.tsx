...
export function ShareDialog({ label, defaultId, kind, build, onClose }: Props) {
  ...
  const download = useCallback(() => {
    ...
    canvas.toBlob((blob) => {
      if (!blob) {
        setNote(t('share.generateFailed'))
        return
      }
      ...
      saveBlob(blob, fileName(kind, id, new Date()), (key) => setNote(t(key)))
    }, 'image/png')
  }, [id, kind])
      ...
      */}
      ...
      >
        <div className="glass-lit cut-card px-7 pt-11 pb-7 text-left">
          ...
          <p className="mt-4 text-lg font-bold text-ink">{t('share.title')}</p>
          <p className="jp-wrap mt-1 text-xs text-ink-sub">{t('share.description')}</p>

          <label className="mt-5 block">
            <span className="text-2xs font-semibold tracking-[0.3em] text-primary">{t('share.idLabel')}</span>
            <span className="mt-2 block">
              <Field
                value={id}
                maxLength={MAX_ID}
                placeholder={t('share.idPlaceholder')}
          ...
          */}
          ...
          <p role="status" aria-live="polite" className="jp-wrap mt-3 min-h-[1.5em] text-xs text-ink-sub">
            {note ?? (ready ? t('share.saveHint') : t('share.generating'))}
          </p>
...
function saveBlob(
  blob: Blob,
  name: string,
  setNote: (key: 'share.downloadStarted' | 'share.downloadBlocked') => void,
): void {
  ...
  try {
    ...
    setNote('share.downloadStarted')
  } catch {
    if (url) URL.revokeObjectURL(url)
    setNote('share.downloadBlocked')
  }
}
