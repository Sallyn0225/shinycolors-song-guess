...
| `apps/web` 单机 + 联机 1v1 UI | ✅ 可玩 | 87 |

**278 个测试全过，全仓 `tsc --noEmit` 干净。**
...
- **全员掉线的房间会在列表里挂满 30 分钟**。掉线只 `detach` 不清座位（要留给重连），
  ...
  有了列表之后它会显示成一个能加入的活房间。现在**一条活连接都不剩就当场回收**
  （`Hub.dropIfDeserted`，判据 `allOffline`），座位凭证同时作废；
  `ABANDONED_TTL_MS`（断线宽限 + 5 秒）退居为清扫兜底，正常路径不该命中它。
