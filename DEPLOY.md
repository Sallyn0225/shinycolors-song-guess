...
| 变量 | 默认 | 说明 |
|---|---|---|
| `MAX_ROOMS` | `200` | 全局同时存在的房间数上限。**设成 `0` 即临时关停建房**，不用改代码 |
...
| `ABANDONED_TTL_MS` | `65000` | 全员掉线的房间还留多久。**不能小于 60 秒的断线宽限**，小了就等于取消重连 |
...
本文开头那条警告在这里加倍适用 —— 切片是商用音源的派生物。公开房间列表意味着
任何人都能进来听。**这是一个知情选择，不是一个技术细节。**
...
没有数据库，房间全在内存里，**服务重启即全部消失**。三种自动回收：

| 情况 | 何时回收 |
|---|---|
| 座位空了（都点了「离开房间」） | 下一轮清扫（≤ 5 秒） |
| 全员掉线 | `ABANDONED_TTL_MS`（默认 65 秒，即断线宽限 + 5 秒） |
