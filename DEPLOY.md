...
> ⚠️ **版权**：切片是 2.0GB 商用音源的派生物。**代码是公开的，曲库不是**——
...
assets/    240MB，本地构建好之后传上去
```

**`songs/` 绝对不要传。** 2.0GB 的源音源，服务端根本不读它 ——
...
无需额外动作。留在 VPS 由本进程伺服在当前体量下也没有代价：全部 272 张
共 2.1MB、单张 ~8KB，且带长缓存头，玩家重复访问不重复下载。
...
3. 批量预下载全部 1632 个切片、离线建对照表，是这个游戏原理上唯一挡不住的攻击。
