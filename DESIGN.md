...
文字排到边没有落脚处。先收到 `1120u`，再收到 `900u`。四档实测的单侧留白：

| 视口 | `--u` | 版心 1120u → 900u | 留白 |
|---|---|---|---|
| 1366×678 | 0.78 | 874 → 702 | 18.0% → 24.3% |
| 1440×810 | 0.9 | 1008 → 810 | 15.0% → 21.9% |
| 1536×774 | 0.86 | 963 → 774 | 18.6% → 24.8% |
| 1920×990 | 1.0 | 1120 → 900 | 20.8% → 26.6% |

第二次收窄的理由是**横向扫视距离**，不是留白比例：1120u 在 1366 档给出 874px 的版心，
首页那条难度横条左端是「简单」、右端是四项参数，隔着约 600px 的空白，读一档难度要横扫整行。

这里原先写着「再窄就不行——选项条是『整宽横条』的形状语汇，1366 档再收会短到失去这个语义」。
**实测否掉了这句话**：900u 在 1366 档的选项条是 624 × 75px，宽高比 8.3 : 1
（1120u 是 796 × 75 = 10.6 : 1），横条依然是横条。

也不必担心它吃掉纵向预算：六档实测 Start 与 Play 的 `scrollHeight` 在两个版心下**逐档相等**。
原因是 `.sc-bar` 与 `.sc-revealslot` 都是定高（The Constant-Height Rule），
而首页难度条的说明文字在 900u 下仍是单行。

再往下收才需要重新量：`760u` 就是 `--page-narrow`，那时选项条与大厅表单同宽，
「猜歌」与「填表」会开始长得像同一件事。
...
**The Capped-List Rule.** 一份长度由数据决定的列表，不铺在页面流里，装进定高的滚动区。
结算页的逐题列表原来是直铺的，实测 1366×678 上 10 题溢出 464px、20 题 1061px，
而底部那排按钮分别落在折线下 383px 与 979px——看完成绩想再来一局，要先滚过整份曲目表。

上限写成 `min(calc(N * var(--u)), Xdvh)`：只用 `--u`，矮窗口上仍会把动作挤下去；
只用 `dvh`，超高窗口上列表长回去、限高失去意义。这跟 `ShareDialog` 给预览图设上限是同一条做法。
**验收标准是「两种题数的 `scrollHeight` 相等」**——「页面不再被列表拉长」没有别的可测量定义。

滚动区要说出「这里面还能滚」，用上下缘的 `mask-image` 渐隐，不用框线（区块之间靠留白与斜切分开）
也不用 `clip-path`（硬边做不出淡出，且会和行自己的裁剪叠加）。在手机这类没有占位滚动条的平台上，
这道渐隐是唯一的信号。代价见上面 The Lifted-Outline Rule 补的那一段。
...
**凶手不止 `clip-path`：`mask-image` 一样吃 outline。** mask 的绘制区默认是 border-box，
而 outline 画在 border-box 之外，落在遮罩范围外就是全透明。这一条比 `clip-path` 那条更难发现——
`getComputedStyle` 照样报 `outline: rgb(0,119,168) solid 2px`，`:focus-visible` 也照样 match，
只有截图上什么都没有。结算页那个渐隐滚动区就是这么被抓到的（先写成一层、截图对不上才发现）。
**任何带 `mask-*` 的可聚焦元素，环也要提到不带 mask 的外层。**
