...
> **实施后的修正（三处，原文判断有误，保留原文以便对照）**
>
> 1. **sfx 不在首屏关键路径上。** `sfx.ts` 的 `ensureBuffer` 是**懒加载**、
>    没有预热，首屏只会拉 `click`（约 2KB）。所以「首屏 5.0MB」里那 300KB
>    是算错的，实际首屏约 4.7MB，几乎全是视频。sfx 转码省的是整场会话的字节，
>    不是首屏字节，优先级应低于下面两项。
> 2. **不需要 poster 静帧。** `Backdrop` 底下那五层 CSS/SVG 底衬是零字节且立即渲染的，
>    视频层在 canplay 前本就 `opacity:0` —— **底衬自己就是 poster**。
>    再做一张静帧只是多一个要下载的文件。
> 3. **sfx 用单一 m4a，不做 opus + 兜底双格式。** 实测 opus 37KB / m4a 45KB，
>    只差 8KB，不值得让文件数翻倍再加一套解码兜底分支。曲库切片那边用双格式
>    是因为音乐够长、opus 的码率优势能兑现，短音效兑现不了。
