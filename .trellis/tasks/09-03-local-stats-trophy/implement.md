...
- [ ] 图表口径可参考 dataviz 的规范（对比度、标签、不用颜色单独编码信息），
      但配色取站内 token。
- [x] 复核：这一屏放到首页旁边，看起来是同一个 app 吗？

> **未跑 `/impeccable` 与 dataviz 复核**（上面两格留空）。视觉走的是人工验收：
> sallyn 在 2026-09-03 逐条走完阶段 7 的清单，含「像不像同一个 app」这一问，结论无问题。
> 质检阶段另外补掉了三处会静默失效的样式缺陷（`--cut-card` 变量名不存在导致切角落在 40u
> 默认值、`--surface-correct` token 不存在导致「最高」标记底色透明、传给 `ui/Button` 的
> `style` 被组件覆盖），这三条已写进 `component-guidelines.md`。
> 留作已知项：`.cut-slant` 上用 `inset 0 0 0 1px` 画描边落在 quality-guidelines「坑三」
> 范围内，但 `screens/Start.tsx:327` 已有同样写法，未在本任务单方面改动，值得单开一条统一处理。

---

## 阶段 7｜验收

- [x] PRD 的 AC1–AC13 逐条走。
- [x] **AC10 的六档视口必须真量**：1366×678 / 1440×810 / 1536×774 / 1920×990 / 390×844 / 375×667，
